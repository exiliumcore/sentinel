...
@pytest.fixture
def valid_exilium_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'ETqEqefnCaZ1VK1yJEziz3Zya6ZqtQdd8J'


@pytest.fixture
def invalid_exilium_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'ETqEqefnCaZ1VK1yJEziz3Zya6ZqtQdd8e'
...
def test_valid_exilium_address():
    from exiliumlib import is_valid_exilium_address

    main = valid_exilium_address()
    test = valid_exilium_address('testnet')

    assert is_valid_exilium_address(main) is True
    assert is_valid_exilium_address(main, 'mainnet') is True
    assert is_valid_exilium_address(main, 'testnet') is False

    assert is_valid_exilium_address(test) is False
    assert is_valid_exilium_address(test, 'mainnet') is False
    assert is_valid_exilium_address(test, 'testnet') is True


def test_invalid_exilium_address():
    from exiliumlib import is_valid_exilium_address

    main = invalid_exilium_address()
    test = invalid_exilium_address('testnet')

    assert is_valid_exilium_address(main) is False
    assert is_valid_exilium_address(main, 'mainnet') is False
    assert is_valid_exilium_address(main, 'testnet') is False

    assert is_valid_exilium_address(test) is False
    assert is_valid_exilium_address(test, 'mainnet') is False
    assert is_valid_exilium_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from exiliumlib import elect_mn
...
def test_parse_masternode_status_vin():
    from exiliumlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % exiliumlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(exiliumlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(exiliumlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(exiliumlib.blocks_to_seconds(16616)) == 2612035
