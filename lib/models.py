...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, exiliumd):
        golist = exiliumd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_exiliumd(exiliumd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_exiliumd(self, exiliumd, rec):
        import decimal
        import exiliumlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from exiliumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from exiliumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from exiliumd - Exiliumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(exiliumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from exiliumd! %s" % e)
            govobj.vote_delete(exiliumd)
            return (govobj, None)
    ...
    def vote_delete(self, exiliumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(exiliumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, exiliumd, signal, outcome):
        import exiliumlib
        ...
        output = exiliumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = exiliumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(exiliumd, signal)

    def sync_network_vote(self, exiliumd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = exiliumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = EXILIUMD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 exilium addr, non-multisig
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (exiliumlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = EXILIUMD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not exiliumlib.is_valid_exilium_address(addr, config.network):
    ...
    def hash(self):
        import exiliumlib
        return exiliumlib.hashit(self.serialise())
